...
org.todesschaf.cacheUsage.onProfileDisappearing = function(subject, data) {
    ...
    // Finally, write our data to our output file
    var ostream = file.open(this.outputFile, "wb");
    ostream.write(JSON.stringify(cacheUsageData));
    ostream.close();
};

org.todesschaf.cacheUsage.makeComponent = function(num) {
    var res = "_";
    if (num < 10) {
        res += "0";
    }
...
org.todesschaf.cacheUsage.setup = function() {
    ...
    outFileName += org.todesschaf.cacheUsage.makeComponent(month);
    outFileName += org.todesschaf.cacheUsage.makeComponent(dd);
    outFileName += org.todesschaf.cacheUsage.makeComponent(hh);
    outFileName += org.todesschaf.cacheUsage.makeComponent(mm);
    outFileName += org.todesschaf.cacheUsage.makeComponent(ss) + ".json";
